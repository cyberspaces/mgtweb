...
//  }
  ...
  def getAppUsageStats()=JsonpAction{(request,cookie)=>
    val starttime=request.getQueryString("st")
    val endtime=request.getQueryString("et")
    ...
    val sql=request.getQueryString("country") match{
      case Some(country)=>s"select * from $tableName where country='$country' and time>=$starttime and time <=$endtime limit ${page*max},$max"
      case None=>s"select * from $tableName where time>=$starttime and time <=$endtime limit ${page*max},$max"
    }
