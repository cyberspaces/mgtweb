  ...
  override def aopAction(request: RestRequest): Response = {
    ...
    val user = SlickDBPoolManager.DBPool.withTransaction { implicit session =>
      u_type match{
        ...
        case _=>throw new RestException(RestRespCode.Invalid_authorization_parameters,"无效的账号类型")
      }
    }
    val content = {
      user match {
          ...
          if (u.utype.equals(uType.get)) {
            u.status match {
              case AccountUtil.account_status_normal | AccountUtil.account_status_inactive | AccountUtil.account_status_supervise=>
                ...
                RestResponseContent(RestRespCode.succeed, token)
              case AccountUtil.account_status_freeze =>
                RestResponseContent(RestRespCode.login_user_freeze, "账号暂时被冻结")
              case AccountUtil.account_status_delete=>
                RestResponseContent(RestRespCode.login_user_delete, "账号已经被删除")
              case AccountUtil.account_status_suicide=>
                RestResponseContent(RestRespCode.login_user_suicide,"账号已经注销")
            }
          } else {
            RestResponseContent(RestRespCode.invalid_request_parameters, "未找到账号类型为" + uType + "的账号")
          }
        case None => RestResponseContent(RestRespCode.user_not_exit, "不匹配任何账户")
      }
    }
...
class UserRegisterTokenAction extends RestAopRouterProvider{
  ...
  override def aopAction(request: RestRequest): Response = {
    ...
    IsAccountExist(u_type, u_name) match {
      case Some(u) => throw new RestException(RestRespCode.already_user_account, "账号已经存在")
      case None => {
        val passwd = registerMap.get("password") match {
          case Some(x) => x
          case None => throw new RestException(RestRespCode.Invalid_authorization_parameters, "密码不能为空")
        }
        val uType = registerMap.get("uType") match {
          case Some(x) => x
          case None => throw new RestException(RestRespCode.Invalid_authorization_parameters, "注册账号类型不能为空")
        }
        val (username, email, phone) = u_type match {
          ...
          case _ => throw new RestException(RestRespCode.Invalid_authorization_parameters, "无效的注册账号类型")
        }
        ...
        val content = RestResponseContent(RestRespCode.succeed, token)
        DefaultHttpResponse.createResponse(content)
      }
    }
  }
  ...
  def decodeUserAccount(account:Option[String]) :(String,String)={
    account match{
      case Some(u)=>
        ...
        if(a_start<0 || a_start == u.length-1) throw new RestException(RestRespCode.Invalid_authorization_parameters,"无效的用户名格式")
        (u.substring(0,a_start),u.substring(a_start+1))
      case None=>throw new RestException(RestRespCode.Invalid_authorization_parameters,"无效的用户名")
    }
  }
}
