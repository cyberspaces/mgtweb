...
class NotFindActionException(url:String,error:String) extends RestException(RestRespCode.no_such_method,"Not Find Method ["+url+"],Error Message["+error+"]")
class NoAuthException(url:String,error:String) extends RestException(RestRespCode.permission_need,"This Method ["+url+"] Need User Permission,Error Message["+error+"]")
class RestDBException(message:String,ex:Throwable) extends RestException(RestRespCode.service_inline_cause,message,ex)
class RestCtlException(message:String,ex:Throwable) extends RestException(RestRespCode.service_inline_cause,message,ex)
