...
object SpiderActionInspectorFilterService extends SimpleFilter[Request,Response]{
  ...
  override def apply(request: Request, service: Service[Request, Response]): Future[Response] = {
    ...
    if(TokenUtil.validateIsHackAction(remote)){
      ...
      val content=RestResponseContent(RestRespCode.App_over_invocation_limit,"请不要频繁调用接口，你调用次数超过限制")
  ...
  override def apply(request: Request, service: Service[Request, Response]): Future[Response] = {
    service(request) handle{
      ...
      case ex:TimeoutException=>
        val content=RestResponseContent(RestRespCode.service_execution_timeout,"服务器运行超时,请稍后重试")
        errorLog.error("FullErrorStack",ex)
        DefaultHttpResponse.createResponse(content,None)
      case ex:Throwable=>
        val content=RestResponseContent(RestRespCode.service_inline_cause,ex.getMessage)
        errorLog.error("FullErrorStack",ex)
        DefaultHttpResponse.createResponse(content,None)
    }
  }
...
object TrackerInfo{
  def apply(request:Request):Map[String,String]={
    Map("ip"->request.remoteHost,"method"->request.getMethod().getName,"url"->request.getUri(),"server_id"->GlobalConfig.server_id,"tracker_id"->request.headers().get("Tracker_Id"))
  }
}
