...
trait AuthAopAction extends RestAopAction{
  abstract override def aopAction(request:RestRequest):Response={
    ...
    if(!findAccessToken(request)) throw new RestException(RestRespCode.invalid_token,"Invalid Token")
  ...
  def findAccessToken(request:RestRequest): Boolean ={
    ...
    if(token == null || userId == null || clientId==null || tokenType ==null) throw new RestException(RestRespCode.Invalid_authorization_parameters,"Invalid Auth Parameters")
...
class TestBaseInfoAopAction extends RestAopRouterProvider{
  override def aopAction(request: RestRequest): Response = {
    val content=RestResponseContent(RestRespCode.succeed,"base info")
